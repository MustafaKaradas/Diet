...
namespace DietProgram.UI.AppUI
{
    ...
    {
        private readonly IUserFoodInfoService _userFoodInfoService;
        private readonly IFoodService _foodService;
        DietProgramDbContext _context;
        public int id { get; set; }
        public frmDinner(IFoodService foodService, IUserFoodInfoService userFoodInfoService)
        {
            InitializeComponent();
            _context = new DietProgramDbContext();
            _foodService = foodService;
            _userFoodInfoService = userFoodInfoService;
        }
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                dataGridViewSearch.DataSource = foods.ToList();
                if (txtSearch.Text.Length == 0)
                {



                    dataGridViewSearch.DataSource = _foodService.GetAll().Data.ToList();
                }
            }
        }
        ...
        {
            int id =this.id;
            string foodName = dataGridViewSearch.SelectedRows[0].Cells["FoodName"].Value.ToString();
            double kcal = Convert.ToInt32(dataGridViewSearch.SelectedRows[0].Cells["Kcal"].Value);
            double protein = Convert.ToInt32(dataGridViewSearch.SelectedRows[0].Cells["Protein"].Value);
            double carbonhydrate = Convert.ToInt32(dataGridViewSearch.SelectedRows[0].Cells["Carbonhydrate"].Value);
            double fat = Convert.ToInt32(dataGridViewSearch.SelectedRows[0].Cells["Fat"].Value);
            ...
            UserFoodInfoCreateVm userFoodInfoCreateVm = new UserFoodInfoCreateVm()
            {
                ...
                MealName = lblDinner.Text,
                Portion = portion,
                TakenCalorie = takenCalorie,
                
            };

            _userFoodInfoService.Create(userFoodInfoCreateVm);
            MessageBox.Show("Öğüne Başarıyla Eklendi");
            string mealName = lblDinner.Text;
            dgvSelectedFood.DataSource = _userFoodInfoService.GetAll(mealName, id).Data.ToList();
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            _userFoodInfoService.Delete(id);
