...
{
    public partial class frmBreakfastPage : Form
    {
        ...
        public frmBreakfastPage(IFoodService foodService, IUserFoodInfoService foodInfoService)
        {
            InitializeComponent();
        ...
        private void frmBreakfastPage_Load(object sender, EventArgs e)
        {
        ...
        {
            int id = this.id;
            string foodName = dgvBreakfast.SelectedRows[0].Cells["FoodName"].Value.ToString();
            double kcal = Convert.ToInt32(dgvBreakfast.SelectedRows[0].Cells["Kcal"].Value);
            double protein= Convert.ToInt32(dgvBreakfast.SelectedRows[0].Cells["Protein"].Value);
            double carbonhydrate= Convert.ToInt32(dgvBreakfast.SelectedRows[0].Cells["Carbonhydrate"].Value);
            double fat= Convert.ToInt32(dgvBreakfast.SelectedRows[0].Cells["Fat"].Value);
            int portion = (int)numericUdPortions.Value;
            ...
            string mealName = lblBreakfast.Text;
            dgvBreak.DataSource = _foodInfoService.GetAll(mealName, id).Data.ToList();
        }
        ...
        private void btnAll_Click(object sender, EventArgs e)
        {
            ...
            dgvBreakfast.DataSource = foods.ToList();
        }
        ...
        {
            var foods = _context.Foods.Where(x => x.FoodName == textBox6.Text).Select(x => new
            {
                x.Id,
                x.FoodName,
                x.Kcal,
            ...
            var newData = _foodService.FindFoodByName(textBox6.Text);
            if (newData.HasError)
            {
            ...
            else
            {
                ...
                if (textBox6.Text.Length == 0)
                {
                    ...
                    dgvBreakfast.DataSource = _foodService.GetAll().Data.ToList();
                }
            }
        }
        ...
        {
            string foodNames = dgvBreak.SelectedRows[0].Cells["FoodName"].Value.ToString();
